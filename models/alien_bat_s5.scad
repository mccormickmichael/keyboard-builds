...
// Alien bat section 5: bottom left section
...
difference() {
  ...
  for (i = [0:2]) {
    keyhole(-plat_x/2 + border + m_key_cell_x/2, plat_y/2 - i*key_cell - key_cell/2);
  }
  for (i = [0:1]) {
    keyhole(plat_x/2 - key_cell/2, plat_y/2 - i*key_cell - key_cell/2);
  }
  for (i = [0:1]) {
    keyhole(plat_x/2 - key_cell - key_cell/2, plat_y/2 - i*key_cell - key_cell/2);
  }
